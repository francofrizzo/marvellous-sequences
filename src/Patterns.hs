...
candidate2PatternsSeeds n m =
  map
    (\(x, y) -> concat $ zipWith (\v w -> [v, w]) x y)
    [(x, y) | x <- preSeeds, y <- preSeeds, length x == length y]
  where
    permutedPreSeeds = concatMap (nub . permutations) preSeeds
...
limitedPatternsAux lim k n
  | n == 1 = map (\c -> (1, c, [c])) (characters k)
  | n > 1 =
    concatMap
